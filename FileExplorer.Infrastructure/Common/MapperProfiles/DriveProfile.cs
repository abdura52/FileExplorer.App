...
public class DriveProfile : Profile
{
    public DriveProfile()
    {
        CreateMap<DriveInfo, StorageDrive>()
            ...
            .ForMember(src => src.FreeSpace, opt => opt.MapFrom(dest => dest.TotalFreeSpace))
            .ForMember(src => src.UnavailableSpace, opt => opt.MapFrom(dest => dest.TotalSize - dest.AvailableFreeSpace))
